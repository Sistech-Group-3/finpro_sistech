...
def generate_diverse_routes(
    ...
    edges it used are penalized (weight multiplied by `penalty_factor`) for
    the next search, then the shortest path is recomputed.
    ...
    Penalties are tracked in an out-of-band dict and applied through a
    weight callable, so the input graph is never copied or mutated (a full
    deepcopy of the city-wide walk graph costs tens of seconds per call).
    ...
    # Penalty multipliers per directed edge (u, v), keyed by the node pair.
    # This replaces the old `copy.deepcopy(G)` (tens of seconds on the
    # city-wide graph) — penalties live out-of-band and are applied via a
    # weight callable, so the shared cached graph is never mutated and the
    # function stays thread-safe across concurrent requests.
    penalties: Dict[tuple, float] = defaultdict(lambda: 1.0)

    def _weight(u, v, d):
        return d.get(weight, 1.0) * penalties[(u, v)]
    ...
    for _ in range(n_routes):
        try:
            node_path = nx.shortest_path(G, orig, dest, weight=_weight)
        ...
        if key in seen_node_sets:
            ...
            _penalize_edges(penalties, node_path, penalty_factor)
            try:
                node_path = nx.shortest_path(G, orig, dest, weight=_weight)
            except nx.NetworkXNoPath:
                break
        ...
        # Penalize this path's edges so the next iteration is pushed
        # toward a genuinely different route
        _penalize_edges(penalties, node_path, penalty_factor)

    return routes


def _penalize_edges(penalties: Dict[tuple, float], node_path: list, factor: float):
    """
    Multiply the recorded penalty for every directed edge (u, v) used in
    node_path by `factor`. Applied per node pair (covers all parallel
    edges between u and v), matching the old in-place mutation semantics.
    """
    for u, v in zip(node_path[:-1], node_path[1:]):
        penalties[(u, v)] = penalties.get((u, v), 1.0) * factor
